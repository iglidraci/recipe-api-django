...
class PrivateRecipeApiTest(TestCase):
    ...
    def test_filter_recipes_distinct(self):
        """
        test returning a recipe only once even if it is in both tags or ingredients
        """
        recipe = sample_recipe(user = self.user, title='Shrimps and pasta')
        tag1 = sample_tag(user = self.user, name = 'Sea')
        tag2 = sample_tag(user=self.user, name='easy')
